...
class Data():
    ...
    def shuffle_data(self):
        indices = list(range(len(self.s1s)))
        np.random.shuffle(indices)
        self.s1s = np.array(self.s1s)[indices]
        self.s2s = np.array(self.s2s)[indices]
        self.labels = np.array(self.labels)[indices]
        self.features = np.array(self.features)[indices]
        # self.ml_features = np.array(self.ml_features)[indices]
    ...
    def next_batch(self, batch_size):
        ...
        # batch_ml_features = self.ml_features[self.index:self.index + batch_size]
    ...
    def get_data(self):
        ...
        # batch_ml_features = self.ml_features[:]
...
class Med(Data):
    def open_file(self, mode, parsing_method="NLTK"):
        # self.ml_features = joblib.load('ml_features.pkl')
        # if mode == 'train':
        #     self.ml_features = self.ml_features[:750]
        # else :
        #     self.ml_features = self.ml_features[750:]

        f = open("./Med_Corpus/" + mode + '-label' + ".txt", "r", encoding="utf-8")
        labels = f.readlines()
        f.close()

        with open("./Med_Corpus/Med-" + mode + ".txt", "r", encoding="utf-8") as f:
            lines = f.readlines()
